...
var url_status = [];
for(var i=0; i<255; i++){
    url_sys_inf[i] = "10.0.2."+i+"/cgi-bin/get_system_info.cgi/";
    url_status[i] = "10.0.2."+i+"/cgi-bin/minerStatus.cgi"
}

readOverview = function(host, port, username, password, callback) {
...
readStats = function(host, port, username, password, callback) {
  ...
  readStats('10.0.2.34', 80, 'root', 'root', (err, stats) => {
    if(err) {
  ...
  readOverview('10.0.2.34', 80, 'root', 'root', (err, overView) => {
    if(err) {
        console.error("\nAn error occurred fetching stats from antminer", err);
    } else {
