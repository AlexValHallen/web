...
async function showStatsSmol(chatId){
    ...
    try{
        ...
        var fssSmolMiners = await connectSmolDB.query("SELECT COUNT(*) FROM smol_v4.minerstat WHERE Type<>'error';", { type: connectSmolDB.QueryTypes.SELECT });
        fssSmolMiners = Object.values(fssSmolMiners[0]);
        fssSmolMiners = parseInt(fssSmolMiners[0], 10);

        // Количество живых плат в Смоленщине
        var fssSmolChains = await connectSmolDB.query("SELECT SUM(AliveChains) FROM smol_v4.otherinfo WHERE IpAddr IN (SELECT IpAddr FROM smol_v4.minerstat WHERE Type<>'error');", { type: connectSmolDB.QueryTypes.SELECT });
        fssSmolChains = Object.values(fssSmolChains[0]);
        fssSmolChains = parseInt(fssSmolChains, 10);

        // Хешрейт в Смоленщине
        var fssSmolRate = await connectSmolDB.query("SELECT SUM(RtHashrate) FROM smol_v4.otherinfo WHERE IpAddr IN (SELECT IpAddr FROM smol_v4.minerstat WHERE Type<>'error');", { type: connectSmolDB.QueryTypes.SELECT });
        fssSmolRate = Object.values(fssSmolRate[0]);
        fssSmolRate = parseFloat(fssSmolRate[0], 10);

        // Прочекаем немайнящие майнеры
        var fssUselessSmol = await connectSmolDB.query("SELECT IpAddr FROM smol_v4.otherinfo WHERE RtHashrate=0 AND AvgHashrate=0;", { type: connectSmolDB.QueryTypes.SELECT});
        var resUselessSmol = [];
        ...
        var fssLUpdSmol = await connectSmolDB.query("SELECT updatedAt FROM smol_v4.minerstat LIMIT 1");
...
async function showStatsOlymp(chatId){
    ...
    try{
        ...
        var fssOlympMiners = await connectOlympDB.query("SELECT COUNT(*) FROM olymp.minerstat WHERE Type<>'error';", { type: connectOlympDB.QueryTypes.SELECT});
        ...
        var fssOlympChains = await connectOlympDB.query("SELECT SUM(AliveChains) FROM olymp.otherinfo WHERE IpAddr IN (SELECT IpAddr FROM olymp.minerstat WHERE Type<>'error');", { type: connectOlympDB.QueryTypes.SELECT});
        fssOlympChains = Object.values(fssOlympChains[0]);
        fssOlympChains = parseInt(fssOlympChains, 10);

        // Хешрейт в Олимпийском
        var fssOlympRate = await connectOlympDB.query("SELECT SUM(RtHashrate) FROM olymp.otherinfo WHERE IpAddr IN (SELECT IpAddr FROM olymp.minerstat WHERE Type<>'error');", { type: connectOlympDB.QueryTypes.SELECT});
        fssOlympRate = Object.values(fssOlympRate[0]);
        fssOlympRate = parseFloat(fssOlympRate[0], 10);

        // Прочекаем немайнящие майнеры
        var fssUselessOlymp = await connectOlympDB.query("SELECT IpAddr FROM olymp.otherinfo WHERE RtHashrate=0 AND AvgHashrate=0;", { type: connectOlympDB.QueryTypes.SELECT});
        var resUselessOlymp = [];
        ...
        // Проверим время последнего апдейта в БД
        var fssLUpdOlymp = await connectOlympDB.query("SELECT updatedAt FROM olymp.minerstat LIMIT 1");
