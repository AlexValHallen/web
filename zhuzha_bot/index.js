...
async function alert(){
    ...
    try{
        ...
        // Если из сети вышел хоть 1, сигналим
        ...
        // Хешрейт в Олимпийском
        var HRateOlymp = await connectOlympDB.query("SELECT SUM(RtHashrate) FROM olymp_v2.otherinfo WHERE IpAddr IN (SELECT IpAddr FROM olymp_v2.minerstat WHERE Type<>'error');", { type: connectOlympDB.QueryTypes.SELECT});
    ...
    try{
        ...
        if(absentSmol.length!=0){
            for(authUser in auth_users){
                bot.sendMessage(auth_users[authUser], 'Из сети вышло ' + (absentSmol.length) + ' майнеров в Смоленщине:\n'+ absentSmol);
            }
        }   
        resSmol0 = resSmol;

        // Хешрейт в Смоленщине
        var HRateSmol = await connectSmolDB.query("SELECT SUM(RtHashrate) FROM smol_v5.otherinfo WHERE IpAddr IN (SELECT IpAddr FROM smol_v5.minerstat WHERE Type<>'error');", { type: connectSmolDB.QueryTypes.SELECT });
        ...
        if((decreaseSmol/HRateSmolOld)*100>=5){
            for(authUser in auth_users){
                bot.sendMessage(auth_users[authUser], 'WARNING! Хешрейт упал на ' + decreaseSmol.toLocaleString('ru') + ' GH/S в Олимпийском');
            }
        }
