    ...
    /* var parseUptime = function(uptimeText) {
      ...
      for(var k in urls){
        try {
            ...
            entryID1 = urls[k]+"/cgi-bin/minerStatus.cgi";
            ...
            await req(options1, function (error, resp, body) {
...
exports.readStats = async function(urls){
    for(var j in urls){
            var entryID1 = "http://"+urls[j]+"/cgi-bin/get_system_info.cgi/";
            var entryID2 = "http://"+urls[j]+"/cgi-bin/minerStatus.cgi";
            var options1 = {
                uri: entryID1,
                auth: {
                    user: 'root',
            ...
            try{
                await req(options1, function(error, resp, body){
                    if (!error && resp.statusCode == 200){
                        ...
                        req(options2, function (error2, resp2, body2) {
                            if (!error2 && resp2.statusCode == 200) {
                                var curURL = entryID2;
                                console.log(j+".☻ Receiving stats from "+curURL+"\n");
                                var stats = {};
            
                                const frag = JSDOM.fragment(body2);
                        ...
                        .then(() => {
                                    ...
                                    minerStat.hasMany(DBChains, {foreignKey: 'MinerStat_id', sourceKey: 'id'});
                                    DBChains.belongsTo(minerStat, {foreignKey: 'MinerStat_id', sourceKey: 'id'});
